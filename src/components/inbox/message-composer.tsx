...
export function MessageComposer({
  ...
  // Throttle "typing…" presence pings so we don't spam the API on every
  // keystroke. WhatsApp shows the indicator for a few seconds per ping.
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      adjustHeight();
      const now = Date.now();
      if (value.trim() && now - lastPresenceRef.current > 2500) {
      ...
      {draft ? (
        ...
        <div className="flex items-end gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="max-h-[60vh] overflow-y-auto border-border bg-popover">
              ...
              <DropdownMenuItem onClick={() => setRichKind("location")}>
                <MapPin className="mr-2 h-4 w-4" />
                Location
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setRichKind("contact")}>
