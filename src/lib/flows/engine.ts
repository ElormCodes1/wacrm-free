...
/**
 * Flatten a send_buttons / send_list node into its ordered options.
 * The order MUST match the numbered-text rendering in
 * `flows/meta-send.ts` (buttons in order; list rows continuously across
 * sections) so a "reply with the number" maps to the right option.
 */
function interactiveOptions(node: {
  node_type: string;
  config: Record<string, unknown>;
}): Array<{ title: string; next_node_key: string }> {
  if (node.node_type === "send_buttons") {
    const cfg = node.config as unknown as SendButtonsNodeConfig;
  ...
  if (node.node_type === "send_list") {
    ...
    for (const section of cfg.sections ?? []) {
      for (const r of section.rows ?? []) {
        rows.push({ title: r.title, next_node_key: r.next_node_key });
      }
    }
    return rows;
  }
...
/**
 * Match a free-text reply to a send_buttons / send_list option.
 *
 * On the self-hosted (Baileys) backend, interactive prompts are rendered
 * as a numbered text list rather than native tappable buttons, so the
 * customer replies by typing. We accept either:
 *   - the option's 1-based number ("2", "2.", "2)"), or
 *   - the option's exact title (case-insensitive).
 * Returns the matched option's next_node_key, or null.
 */
export function matchReplyText(
  ...
  if (!text) return null;
  const options = interactiveOptions(node);
  if (options.length === 0) return null;

  // 1-based number, optionally followed by a '.' or ')'.
  ...
  // Exact title match (case-insensitive). Deliberately not a substring
  // match — short labels like "No" would false-match "Not sure".
  const lower = text.toLowerCase();
  const hit = options.find((o) => (o.title ?? "").trim().toLowerCase() === lower);
  return hit?.next_node_key ?? null;
}
...
async function handleReplyForActiveRun(
  ...
  //   2. Numbered/typed text reply to a send_buttons/send_list node —
  //      the free backend renders prompts as numbered text, so the
  //      customer types the number or the option label (see matchReplyText).
  //   3. Text reply on a collect_input node — capture into vars.
  //
  // Everything else falls through to the fallback policy below.
  let matched: string | null = null;
  const onInteractiveNode =
    currentNode.node_type === "send_buttons" ||
    currentNode.node_type === "send_list";
  if (message.kind === "interactive_reply" && onInteractiveNode) {
    matched = matchReplyId(currentNode, message.reply_id);
  } else if (message.kind === "text" && onInteractiveNode) {
