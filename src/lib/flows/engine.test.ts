...
describe("matchReplyText", () => {
  const buttons = {
    ...
    config: {
      ...
      buttons: [
        { reply_id: "yes", title: "Yes", next_node_key: "existing" },
        { reply_id: "no", title: "No", next_node_key: "new" },
      ],
    },
  };
  ...
  const list = {
    ...
    config: {
      ...
      button_label: "View",
      sections: [
        { title: "A", rows: [{ reply_id: "r1", title: "Alpha", next_node_key: "n1" }] },
        {
          title: "B",
          rows: [
            { reply_id: "r2", title: "Beta", next_node_key: "n2" },
            { reply_id: "r3", title: "Gamma", next_node_key: "n3" },
          ],
        },
      ],
    },
  ...
  it("matches a button by 1-based number", () => {
    expect(matchReplyText(buttons, "1")).toBe("existing");
    expect(matchReplyText(buttons, "2")).toBe("new");
    expect(matchReplyText(buttons, "2.")).toBe("new");
    expect(matchReplyText(buttons, " 1) ")).toBe("existing");
  });

  it("matches a button by exact title (case-insensitive)", () => {
    expect(matchReplyText(buttons, "yes")).toBe("existing");
    expect(matchReplyText(buttons, "  NO ")).toBe("new");
  });

  it("does not substring-match short labels", () => {
    // "No" must not match "Not sure" — avoids false positives.
    expect(matchReplyText(buttons, "Not sure")).toBeNull();
  });

  it("numbers list rows continuously across sections", () => {
    expect(matchReplyText(list, "1")).toBe("n1");
    expect(matchReplyText(list, "2")).toBe("n2");
    expect(matchReplyText(list, "3")).toBe("n3");
    expect(matchReplyText(list, "Gamma")).toBe("n3");
  });

  it("returns null for out-of-range numbers, empty, and non-matches", () => {
    expect(matchReplyText(buttons, "9")).toBeNull();
    expect(matchReplyText(buttons, "0")).toBeNull();
    expect(matchReplyText(buttons, "")).toBeNull();
    expect(matchReplyText(buttons, "maybe")).toBeNull();
