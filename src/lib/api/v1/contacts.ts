...
export async function resolveAuditUserId(
  ...
  // An account can have several WhatsApp numbers (multi-number); they
  // all share the same owner, so pick the default (first) config rather
  // than `.maybeSingle()`, which errors on >1 row and would silently
  // drop us to the account-owner fallback.
