...
export type StoreInstance =
  | { instanceName: string; isBusiness: boolean }
...
export async function resolveStoreInstance(): Promise<StoreInstance> {
  ...
  const open = (configs ?? [])
    ...
    .map((c) => c.instance_name as string)
  ...
  for (const instanceName of open) {
    try {
      const prof = await fetchOwnBusinessProfile({ instanceName })
      if (prof.isBusiness) return { instanceName, isBusiness: true }
    } catch {
      /* skip an instance that fails to report */
    }
  }
  return { instanceName: open[0], isBusiness: false }
}
