...
export async function resolveConversationByPhone(
  ...
  // can have several numbers (multi-number); pick the default (first)
  // and tag any new conversation with it so the send routes correctly.
  ...
  const whatsappConfigId = config.id as string;
