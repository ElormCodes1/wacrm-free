...
import { WhatsAppBadge } from '@/components/contacts/whatsapp-badge';
...
/** First page + supporting data the server rendered into the initial HTML. */
export interface ContactsInitial {
  contacts: ContactWithTags[];
  totalCount: number;
  tagsMap: Record<string, Tag>;
}

export function ContactsClient({ initial }: { initial: ContactsInitial }) {
  const supabase = createClient();
  const canEdit = useCan('send-messages');
  const canEditSettings = useCan('edit-settings');

  const [contacts, setContacts] = useState<ContactWithTags[]>(initial.contacts);
  // Seeded from the server, so the first paint shows rows (no spinner). A
  // background refresh runs on mount; its spinner is suppressed while we
  // already have rows (see the table body below).
  ...
  const [tagsMap, setTagsMap] = useState<Record<string, Tag>>(initial.tagsMap);
                ...
                >
                  ...
                  <TableCell className="text-foreground font-medium">
                    <span className="inline-flex items-center gap-2">
                      <span className="flex size-6 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-muted text-[10px] font-medium text-muted-foreground">
                        {contact.avatar_url ? (
                  ...
                  <TableCell className="text-muted-foreground font-mono text-xs">
                    <span className="inline-flex items-center gap-1.5">
                      {contact.phone}
                      <WhatsAppBadge status={contact.is_on_whatsapp} />
                    </span>
                  </TableCell>
