...
/** Selected board the server rendered into the initial HTML. */
export interface PipelinesInitial {
  pipelines: Pipeline[];
...
export function PipelinesClient({ initial }: { initial: PipelinesInitial }) {
  ...
  // Seeded from the server (the selected board), so the first paint shows
  // the columns; the mount effects refresh in the background.
  const [pipelines, setPipelines] = useState<Pipeline[]>(initial.pipelines);
  const [selectedPipelineId, setSelectedPipelineId] = useState<string>(
    initial.selectedPipelineId,
  );
