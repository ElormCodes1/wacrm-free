...
import { sendReaction } from '@/lib/whatsapp/provider/evolution';
...
export async function POST(request: Request) {
  try {
    ...
    // Send the reaction from the number this conversation is on.
    ...
    const toDigits = sanitizePhoneForMeta(contact.phone).replace(/\D/g, '');
    ...
    try {
      await sendReaction({
        ...
        // The reaction key references the TARGET message: fromMe is true
        // when we sent that message (agent/bot), false when the customer did.
        fromMe:
      ...
      return NextResponse.json(
        { error: `WhatsApp reaction failed: ${message}` },
        { status: 502 },
      );
    }
